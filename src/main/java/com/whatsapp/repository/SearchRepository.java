...
public interface SearchRepository extends JpaRepository<User, Long> {
  ...
  @Query("SELECT DISTINCT c FROM Conversation c " + "JOIN c.participants cp "
  ...
  @Query("SELECT m FROM Message m " + "JOIN m.conversation c " + "JOIN c.participants cp "
      + "WHERE cp.user.id = :userId AND cp.status = 'ACTIVE' " + "AND m.isDeleted = false "
  ...
  @Query("SELECT COUNT(DISTINCT c) FROM Conversation c " + "JOIN c.participants cp "
      + "WHERE cp.user.id = :userId AND cp.status = 'ACTIVE' "
      + "AND (LOWER(c.name) LIKE LOWER(CONCAT('%', :query, '%')) "
      + "OR LOWER(c.description) LIKE LOWER(CONCAT('%', :query, '%')))")
  long countSearchConversations(@Param("userId") Long userId, @Param("query") String query);

  @Query("SELECT COUNT(m) FROM Message m " + "JOIN m.conversation c " + "JOIN c.participants cp "
      + "WHERE cp.user.id = :userId AND cp.status = 'ACTIVE' " + "AND m.isDeleted = false "
      + "AND LOWER(m.content) LIKE LOWER(CONCAT('%', :query, '%'))")
  long countSearchMessages(@Param("userId") Long userId, @Param("query") String query);

  @Query("SELECT COUNT(u) FROM User u " + "WHERE u.status = 'ACTIVE' AND u.id != :userId "
      + "AND (LOWER(u.displayName) LIKE LOWER(CONCAT('%', :query, '%')) "
      + "OR LOWER(u.username) LIKE LOWER(CONCAT('%', :query, '%')) "
      + "OR u.phoneNumber LIKE CONCAT('%', :query, '%'))")
