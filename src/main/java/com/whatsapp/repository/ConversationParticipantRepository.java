...
public interface ConversationParticipantRepository extends JpaRepository<ConversationParticipant, Long> {
	...
	@Query("SELECT cp FROM ConversationParticipant cp JOIN FETCH cp.user JOIN FETCH cp.conversation "
			+ "WHERE cp.conversation.id IN :conversationIds AND cp.user.id = :userId")
	List<ConversationParticipant> findByConversationIdsAndUserId(@Param("conversationIds") List<Long> conversationIds,
			@Param("userId") Long userId);

	@Query("SELECT cp FROM ConversationParticipant cp JOIN FETCH cp.user JOIN FETCH cp.conversation "
			+ "WHERE cp.conversation.id IN :conversationIds")
	List<ConversationParticipant> findByConversationIdIn(@Param("conversationIds") List<Long> conversationIds);

	@Query("SELECT cp FROM ConversationParticipant cp "
			+ "WHERE cp.conversation.id = :conversationId AND cp.user.id = :userId " + "ORDER BY cp.joinedAt DESC")
	...
	@Query("UPDATE ConversationParticipant cp SET cp.clearedAt = :clearedAt "
			+ "WHERE cp.conversation.id = :conversationId AND cp.user.id = :userId")
	int updateClearedAt(@Param("conversationId") Long conversationId, @Param("userId") Long userId,
