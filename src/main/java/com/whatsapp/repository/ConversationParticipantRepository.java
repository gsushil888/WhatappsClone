...
public interface ConversationParticipantRepository
  ...
  List<ConversationParticipant> findActiveParticipants(
      @Param("conversationId") Long conversationId);

  @Query("SELECT cp FROM ConversationParticipant cp JOIN FETCH cp.user "
      + "WHERE cp.conversation.id = :conversationId AND cp.status = :status "
      + "ORDER BY cp.role DESC, cp.joinedAt ASC")
  List<ConversationParticipant> findByConversationIdAndStatus(
      @Param("conversationId") Long conversationId,
  ...
  default Optional<ConversationParticipant> findByConversationIdAndUserId(Long conversationId,
      Long userId) {
    List<ConversationParticipant> participants =
        findAllByConversationIdAndUserId(conversationId, userId);
