...
public class OtpHandler {
  ...
  public AuthDto.LoginResponse handleOtpLogin(String contactInfo, OtpVerification.OtpType otpType,
      String displayName) {
    return handleOtpLogin(contactInfo, otpType, displayName, null);
  }

  public AuthDto.LoginResponse handleOtpLogin(String contactInfo, OtpVerification.OtpType otpType,
      String displayName, String contextMessage) {
    ...
    if (existingOtp.isPresent()) {
      ...
      long timeLeft = Duration.between(LocalDateTime.now(), otp.getExpiresAt()).getSeconds();

      if (timeLeft > 0) {
        return buildOtpResponse(otp.getTempSessionId(), maskedContact, (int) timeLeft,
            otp.getExpiresAt(), "Please wait. OTP already sent to " + maskedContact);
      }
    }
  ...
  private AuthDto.LoginResponse buildOtpResponse(String tempSessionId, String maskedContact,
      int expiresIn, LocalDateTime expiresAt, String message) {
    return AuthDto.LoginResponse.builder().requiresOtp(true).tempSessionId(tempSessionId)
        .otpSentTo(maskedContact).expiresIn(expiresIn).expiresAt(expiresAt).message(message)
        .build();
  }
