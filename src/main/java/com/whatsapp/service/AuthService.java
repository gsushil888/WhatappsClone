...
public class AuthService {
  ...
  public AuthDto.SessionValidationResponse validateSession(Long userId, String sessionId) {
    try {
      ...
      if (sessionId == null) {
        return AuthDto.SessionValidationResponse.builder().valid(false).message("No session found")
            .build();
      }
      ...
      if (sessionOpt.isEmpty()) {
        return AuthDto.SessionValidationResponse.builder().valid(false).message("Session not found")
            .build();
      }
      ...
      return AuthDto.SessionValidationResponse.builder().valid(true).userId(userId)
          .expiresAt(session.getExpiresAt()).deviceType(session.getDeviceType())
          .message("Session is valid").build();

    } catch (Exception e) {
      log.error("[SERVICE] Error validating session", e);
      return AuthDto.SessionValidationResponse.builder().valid(false)
          .message("Session validation failed").build();
    }
  }
  ...
  public AuthDto.LoginResponse login(AuthDto.LoginRequest request) {
    String identifier = Optional.ofNullable(request.getIdentifier())
        .orElseThrow(() -> new AuthException(ErrorCode.AUTH_INVALID_CREDENTIALS,
            "Email, mobile, or username is required"));
  ...
  public AuthDto.RefreshTokenResponse refreshToken(AuthDto.RefreshTokenRequest request) {
    ...
    UserSession session = userSessionRepository.findById(sessionId)
        .filter(s -> s.getUser().getId().equals(userId)
            && s.getStatus() == UserSession.SessionStatus.ACTIVE)
  ...
  private AuthDto.LoginResponse handleOtpLogin(User user, AuthDto.LoginRequest request) {
    ...
    Optional<OtpVerification> existingOtp =
        otpService.findActiveOtpByDevice(contactInfo, otpType, deviceFingerprint);

    if (existingOtp.isPresent()) {
      OtpVerification otp = existingOtp.get();
      long timeLeft =
          java.time.Duration.between(LocalDateTime.now(), otp.getExpiresAt()).getSeconds();

      if (timeLeft > 0) {
        return AuthDto.LoginResponse.builder().requiresOtp(true)
            .tempSessionId(otp.getTempSessionId()).otpSentTo(maskedContact)
            .expiresIn((int) timeLeft).expiresAt(otp.getExpiresAt())
    ...
    return AuthDto.LoginResponse.builder().requiresOtp(true).tempSessionId(tempSessionId)
        .otpSentTo(maskedContact).expiresIn(60).expiresAt(LocalDateTime.now().plusMinutes(1))
        .message("OTP sent to " + maskedContact).build();
  }
  ...
  private User createUser(AuthDto.RegisterRequest request, String username,
    ...
    User user = User.builder().username(username).email(request.getEmail())
        ...
        .aboutText(request.getAboutText()).profilePictureUrl(profilePictureUrl)
        .status(User.UserStatus.ACTIVE).accountType(User.AccountType.PERSONAL).isOnline(false)
        .isVerified(false).emailVerified(false).phoneVerified(false).build();
  ...
  private AuthDto.RegisterResponse sendRegistrationOtp(User user, String generatedUsername) {
    String tempSessionId = generateTempSessionId();
    String otpCode = otpService.generateOtp(tempSessionId, user.getEmail(),
    ...
    return AuthDto.RegisterResponse.builder().tempSessionId(tempSessionId)
        .otpSentTo(maskEmail(user.getEmail())).expiresIn(60)
        .expiresAt(LocalDateTime.now().plusMinutes(1)).resendAvailableIn(60)
  ...
  private void updateUserAfterOtpVerification(User user, OtpVerification otp) {
    ...
    if (otp.getOtpType() == OtpVerification.OtpType.EMAIL_LOGIN
        || otp.getContactInfo().contains("@")) {
  ...
  private AuthDto.RefreshTokenResponse updateSessionTokens(UserSession session, Long userId,
      String sessionId) {
    ...
    return AuthDto.RefreshTokenResponse.builder().accessToken(newAccessToken)
        .refreshToken(newRefreshToken).expiresIn(3600).expiresAt(LocalDateTime.now().plusHours(1))
        .build();
  }
  ...
  private AuthDto.UserInfo mapToUserInfo(User user) {
    return AuthDto.UserInfo.builder().id(user.getId()).username(user.getUsername())
        ...
        .aboutText(user.getAboutText()).isOnline(user.isOnline()).lastSeenAt(user.getLastSeenAt())
        .accountType(user.getAccountType().name()).isVerified(user.isVerified()).build();
  }
