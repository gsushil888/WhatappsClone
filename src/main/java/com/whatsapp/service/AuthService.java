...
public class AuthService {
	...
	private GoogleIdToken.Payload verifyGoogleToken(String idToken) {
		try {
			...
			throw new AuthException(ErrorCode.AUTH_INVALID_TOKEN, "Google token verification failed");
		}
	}
	...
	public AuthDto.RegisterResponse resendOtp(AuthDto.ResendOtpRequest request) {
		...
		String otpCode = otpService.generateOtp(request.getTempSessionId(), oldOtp.getContactInfo(),
				oldOtp.getOtpType());

		emailService.sendOtpEmail(oldOtp.getContactInfo(), otpCode, user.getDisplayName());

		return AuthDto.RegisterResponse.builder().tempSessionId(request.getTempSessionId())
				.otpSentTo(maskEmail(oldOtp.getContactInfo())).expiresIn(60)
				.expiresAt(LocalDateTime.now().plusMinutes(1)).resendAvailableIn(60).username(user.getDisplayName())
				.build();
	}
