...
public class ContactService {
	...
	public UserDto.ContactListResponse getUserContacts(Long userId, boolean favoritesOnly, boolean onlineOnly, int page,
			int limit, Map<String, String> deviceInfo, Map<String, String> headers) {
		...
		List<UserDto.ContactResponse> contactResponses = contacts.stream()
				...
				.filter(contact -> !onlineOnly || contact.getContactUser().isOnline()).map(this::mapToContactResponse)
				.collect(Collectors.toList());

		long totalCount = contactRepository.countUserContacts(userId);

		return UserDto.ContactListResponse.builder().contacts(contactResponses).totalCount((int) totalCount)
				.pagination(UserDto.PaginationInfo.builder().page(page).limit(limit)
						.hasNext(contactResponses.size() == limit).build())
				.build();
	}
	...
	public UserDto.ContactResponse addContact(Long userId, UserDto.AddContactRequest request,
		...
		Contact contact = Contact.builder().user(user).contactUser(contactUser)
				.displayName(request.getDisplayName() != null ? request.getDisplayName() : contactUser.getDisplayName())
				.status(Contact.ContactStatus.ACTIVE).build();
	...
	public UserDto.ContactListResponse searchContacts(Long userId, String query, int limit, int offset) {
		...
		List<UserDto.ContactResponse> contactResponses = contacts.stream().map(this::mapToContactResponse)
				.collect(Collectors.toList());

		return UserDto.ContactListResponse.builder().contacts(contactResponses).totalCount(contactResponses.size())
				.pagination(UserDto.PaginationInfo.builder().page(offset / limit + 1).limit(limit)
						.hasNext(contactResponses.size() == limit).build())
				.build();
	}
	...
	public UserDto.SyncContactsResponse syncContacts(Long userId, UserDto.SyncContactsRequest request) {
		...
		List<UserDto.ContactResponse> foundContacts = foundUsers.stream().filter(user -> !user.getId().equals(userId))
				.map(user -> {
					...
					if (!isExistingContact) {
						// Auto-add as contact
						Contact contact = Contact.builder().user(userRepository.findById(userId).orElseThrow())
								.contactUser(user).displayName(user.getDisplayName())
								.status(Contact.ContactStatus.ACTIVE).build();
						contactRepository.save(contact);
					}

					return UserDto.ContactResponse.builder().contactUserId(user.getId())
							.displayName(user.getDisplayName()).phoneNumber(user.getPhoneNumber())
							.profilePictureUrl(user.getProfilePictureUrl()).isOnline(user.isOnline()).build();
				}).collect(Collectors.toList());

		List<String> foundNumbers = foundUsers.stream().map(User::getPhoneNumber).collect(Collectors.toList());

		List<String> notFoundNumbers = request.getPhoneNumbers().stream().filter(phone -> !foundNumbers.contains(phone))
				.collect(Collectors.toList());

		return UserDto.SyncContactsResponse.builder().foundContacts(foundContacts).notFoundNumbers(notFoundNumbers)
	...
	private UserDto.ContactResponse mapToContactResponse(Contact contact) {
		...
		return UserDto.ContactResponse.builder().id(contact.getId()).contactUserId(contactUser.getId())
				.displayName(contact.getDisplayName()).phoneNumber(contactUser.getPhoneNumber())
				.profilePictureUrl(contactUser.getProfilePictureUrl()).isOnline(contactUser.isOnline())
				.isFavorite(contact.isFavorite()).isBlocked(contact.isBlocked())
				.lastActiveAt(contactUser.getLastActiveAt()).createdAt(contact.getCreatedAt()).build();
	}
}