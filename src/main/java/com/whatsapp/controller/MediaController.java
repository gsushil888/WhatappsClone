...
public class MediaController {
	...
	public ResponseEntity<ApiResponse<MediaDto.MediaUploadResponse>> uploadMedia(
		...
		MediaDto.MediaUploadResponse response = mediaService.uploadMedia(userId,
				file, type, conversationId, compress, generateThumbnail);

		return ResponseEntity
				.ok(ApiResponse.<MediaDto.MediaUploadResponse>builder()
	...
	public ResponseEntity<ApiResponse<MediaDto.MediaResponse>> getMedia(
			Authentication authentication, @PathVariable String mediaId) {

		Long userId = (Long) authentication.getPrincipal();
		MediaDto.MediaResponse response = mediaService.getMedia(userId,
				mediaId);
	...
	public ResponseEntity<ApiResponse<MediaDto.MediaListResponse>> getConversationMedia(
		...
		Long userId = (Long) authentication.getPrincipal();
		MediaDto.MediaListResponse response = mediaService.getConversationMedia(
				userId, conversationId, type, limit, offset);

		return ResponseEntity
				.ok(ApiResponse.<MediaDto.MediaListResponse>builder()
						.success(true).data(response)
						.correlationId(RequestContext.getCorrelationId())
						.build());
	}
