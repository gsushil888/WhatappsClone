...
public class MediaController {
  ...
  public ResponseEntity<ApiResponse<MediaDto.MediaUploadResponse>> uploadMedia(
      ...
      @RequestParam(value = "generate_thumbnail",
          defaultValue = "true") boolean generateThumbnail) {

    Long userId = (Long) authentication.getPrincipal();
    MediaDto.MediaUploadResponse response =
        mediaService.uploadMedia(userId, file, type, conversationId, compress, generateThumbnail);

    return ResponseEntity.ok(ApiResponse.<MediaDto.MediaUploadResponse>builder().success(true)
        .data(response).correlationId(RequestContext.getCorrelationId())
        .message("Media uploaded successfully").build());
  }
  ...
  public ResponseEntity<ApiResponse<MediaDto.MediaResponse>> getMedia(Authentication authentication,
    ...
    return ResponseEntity.ok(ApiResponse.<MediaDto.MediaResponse>builder().success(true)
        .data(response).correlationId(RequestContext.getCorrelationId()).build());
  }

  @DeleteMapping("/{mediaId}")
  public ResponseEntity<ApiResponse<Void>> deleteMedia(Authentication authentication,
      @PathVariable String mediaId) {
    ...
    return ResponseEntity.ok(
        ApiResponse.<Void>builder().success(true).correlationId(RequestContext.getCorrelationId())
            .message("Media deleted successfully").build());
  }
  ...
  public ResponseEntity<ApiResponse<MediaDto.MediaListResponse>> getConversationMedia(
    ...
    Long userId = (Long) authentication.getPrincipal();
    MediaDto.MediaListResponse response =
        mediaService.getConversationMedia(userId, conversationId, type, limit, offset);

    return ResponseEntity.ok(ApiResponse.<MediaDto.MediaListResponse>builder().success(true)
        .data(response).correlationId(RequestContext.getCorrelationId()).build());
  }
}
