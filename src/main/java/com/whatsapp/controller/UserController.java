...
public class UserController {
	...
	public ResponseEntity<ApiResponse<UserDto.UserProfileResponse>> getCurrentUser(
		...
		return ResponseEntity
				.ok(ApiResponse.<UserDto.UserProfileResponse>builder()
	...
	public ResponseEntity<ApiResponse<UserDto.UserProfileResponse>> updateProfile(
		...
		UserDto.UserProfileResponse response = userService.updateProfile(userId,
				request);
	...
	public ResponseEntity<ApiResponse<SearchDto.UniversalSearchResponse>> universalSearch(
		...
		Long userId = (Long) authentication.getPrincipal();
		log.info("Universal search by user: {} with query: {} type: {}", userId,
				q, type);

		SearchDto.UniversalSearchResponse response = searchService
				.search(userId, q, type, limit);

		return ResponseEntity
				.ok(ApiResponse.<SearchDto.UniversalSearchResponse>builder()
						.success(true).data(response)
						.correlationId(RequestContext.getCorrelationId())
						.build());
	}
	...
	public ResponseEntity<ApiResponse<List<UserDto.SessionResponse>>> getUserSessions(
		...
		List<UserDto.SessionResponse> sessions = userService
				.getUserSessions(userId);

		return ResponseEntity
				.ok(ApiResponse.<List<UserDto.SessionResponse>>builder()
