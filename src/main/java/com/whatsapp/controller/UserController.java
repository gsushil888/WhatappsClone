...
public class UserController {
	...
	@GetMapping("/users/me")
	public ResponseEntity<ApiResponse<UserDto.UserProfileResponse>> getCurrentUser(Authentication authentication) {
		...
		return ResponseEntity.ok(ApiResponse.<UserDto.UserProfileResponse>builder().success(true).data(user)
				.correlationId(RequestContext.getCorrelationId()).build());
	}

	@PutMapping("/users/me")
	public ResponseEntity<ApiResponse<UserDto.UserProfileResponse>> updateProfile(Authentication authentication,
		...
		return ResponseEntity.ok(ApiResponse.<UserDto.UserProfileResponse>builder().success(true).data(response)
				.correlationId(RequestContext.getCorrelationId()).build());
	}

	@GetMapping("/search")
	public ResponseEntity<ApiResponse<SearchDto.UniversalSearchResponse>> universalSearch(Authentication authentication,
			@RequestParam String q, @RequestParam(defaultValue = "all") String type,
		...
		return ResponseEntity.ok(ApiResponse.<SearchDto.UniversalSearchResponse>builder().success(true).data(response)
				.correlationId(RequestContext.getCorrelationId()).build());
	}

	@GetMapping("/users/me/sessions")
	public ResponseEntity<ApiResponse<List<UserDto.SessionResponse>>> getUserSessions(Authentication authentication) {
		...
		return ResponseEntity.ok(ApiResponse.<List<UserDto.SessionResponse>>builder().success(true).data(sessions)
				.correlationId(RequestContext.getCorrelationId()).build());
	}
	...
	public ResponseEntity<ApiResponse<Void>> terminateSession(Authentication authentication,
		...
		return ResponseEntity.ok(ApiResponse.<Void>builder().success(true).message("Session terminated successfully")
				.correlationId(RequestContext.getCorrelationId()).build());
	}
}
